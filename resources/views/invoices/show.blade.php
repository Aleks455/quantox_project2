        ...
        @if ($invoices->count()) 
            @foreach ($invoices as $invoice)
                <x-list.field>
                    ...
                    <x-list.item value='overflow-hidden' label='Address'>{{ $invoice->client->address }} </x-list.item>
                    <x-list.item value='overflow-hidden' label='City'>{{ $invoice->client->city }} </x-list.item>
                    ...
                    <div class="flex-wrap  justify-around text-sm">
                        <x-list.button color="gray"> 
                            <a href="{{ route('invoices.show', $invoice->id) }}" class="">Show</a>
                        </x-list.button>
